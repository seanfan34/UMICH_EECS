...
int main (int argc, char **argv) {
 	...
 	int sls_p = size; // Number of scanlines in phi1
 ...
 /* --------------------------- COMPUTATION ------------------------------ */
    ...
    for(i = 0; i < NUM_THREADS; i++) {
 	...
 	work_ranges[NUM_THREADS-1].end = boundary;

 	for(i = 0; i < NUM_THREADS; i++) {
  		pthread_create(&child_threads[i], NULL, calculate_image, &work_ranges[i]);
    }
    for(i = 0; i < NUM_THREADS; i++) {
